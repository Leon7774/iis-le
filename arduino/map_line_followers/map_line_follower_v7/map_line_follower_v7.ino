...
// SYSTEM CONSTANTS (Restored to robust v1 settings)
...
struct NodeAttributes {
    uint8_t type; // Using standard uint8_t to allow taking address in PROGMEM (saves same RAM as bit-field)
};
...
// Full GRAPH adjacency matrix restored to allow BFS pathfinding to succeed
...
// RUNTIME STATE EXPLICIT ARCHITECTURE
...
void execute_system_halt() {
    ...
    while (true) {
        // Enforce safe system loop capture
    }
}
...
void process_node_transition(NodeType type) {
    ...
    if (target_direction == -1) execute_system_halt(); // Graph discrepancy error
    ...
    if (type == NODE_TYPE_BLACK_LINE) {
        ...
        if (rotation_delta != 0) {
            ...
            delay(300); // Restored to 300ms blind spin to clear junction
...
void setup() {
    ...
    // Corrected target node from g_planned_path[-1] out-of-bounds to END_NODE
    if (!compute_shortest_path(g_current_node, END_NODE)) {
        execute_system_halt(); 
    }
    g_last_node_time_ms = millis(); // Enforces 1000ms lockout at startup to prevent false triggers
}
...
void loop() {
    ...
    // Determine target node identity configuration cleanly via PROGMEM lookup
    ...
    if (expected_node_type == NODE_TYPE_WHITE_MARKER) {
        ...
        // Restored robust v1 black junction detection criteria to prevent false triggers on curves
        if ((raw_l_out && raw_r_out) || (raw_l_out && raw_l_in && raw_r_in) || (raw_r_out && raw_r_in && raw_l_in)) {
            if (millis() - g_last_node_time_ms > MIN_NODE_INTERVAL_MS) {
                // Restored v1 debounce threshold of 5 loops
